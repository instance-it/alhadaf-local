...
require_once dirname(__DIR__, 2).'\model\posserviceorder.php';
...
if($isvalidUser['status'] == 1) 
{
	...
	//List Service Order Data
	if($action=='listposserviceorder')   
	{
		...
		$posserviceorders=new posserviceorder();
		...
		o.iscancel,isnull(o.pdfurl,'') as inv_pdfurl
		...
		OR convert(varchar, o.timestamp,100) like :orderdatefilter OR o.totalpaid like :totalpaidfilter OR pm1.personname like :entrypersonfilter 
		...
		if($issidebarflt == 1)
		{
			...
			if($fltmemberid)
			{
				$qry.=" and o.uid like :fltmemberid";
				$parms[':fltmemberid']=$fltmemberid;  
			}

			if($fltfromdate && $flttodate)
			{
				$qry.=" and CONVERT(date,o.timestamp,103) between  CONVERT(date,:fromdate,103) and  CONVERT(date,:todate,103)";
		...
		$posserviceorders=$DB->getmenual($qry,$parms,'posserviceorder');
		...
		if($responsetype=='HTML')
		{
			...
			{
				$i=0;
				foreach($posserviceorders as $ordermaster)
				{
					$id="'".$ordermaster->id."'";
					...
					$htmldata.='<td class="tbl-name"><a href="javascript:void(0)" data-toggle="tooltip" data-placement="top" data-original-title="View Order Detail" onclick="viewserviceorderdata('.$IISMethods->sanitize($id).')">'.$IISMethods->sanitize($ordermaster->orderno).'</a></td>';
					...
					{
						$htmldata.='<span class="btn btn-danger px-2 py-0">Cancelled</span>';
					}
					$htmldata.='</td>';
			...
			{	
				if($page<=0 && sizeof($posserviceorders)<=0)
                {
		...
		{
			$response['data']= json_encode($posserviceorders);
		}

		$common_listdata=$posserviceorders;
	}  
	//View Service Order Data
	else if($action=='viewserviceorderdata')   
	{
		$id=$IISMethods->sanitize($_POST['id']);
		
		$mainqry = "select o.timestamp as primary_date,o.id,o.transactionid,o.orderno,pm.personname,pm.contact,
		convert(varchar, o.timestamp,100) AS ofulldate,o.totalpaid,pm1.personname as entrypersonname,pm1.contact as entrypersoncontact,s.storename,
		o.iscancel
		from tblserviceorder o 
		inner join tblserviceorderdetail od on od.orderid=o.id 
		inner join tblpersonmaster pm on pm.id=o.uid 
		inner join tblpersonmaster pm1 on pm1.id=o.entry_uid 
		inner join tblstoremaster s on s.id=o.storeid 
		where o.id = :id";
		...
		if($responsetype=='HTML')
		{
			...
			$htmldata.='<div class="col-12 col-lg-6 col-xl-6 "><div class="row">';
			$htmldata.='<div class="col-12 col-sm-4 col-lg-5 text-dot-right pr-sm-0">';
			$htmldata.='<label>Order Status <span>:</span></label>';
			$htmldata.='</div>';
			$htmldata.='<div class="col-12 col-sm-8 col-lg-6">';
			if($row['iscancel'] == 1)
			{
			...
			$qryod="select sod.id,sod.orderid,sod.type,sod.typename,sod.catid,sod.category,sod.subcatid,sod.subcategory,sod.itemid,sod.itemname,sod.qty,sod.issued_qty AS issuedqty,sod.remain_qty as remainqty,
				sod.taxtype,sod.taxtypename,sod.sgst,sod.cgst,sod.igst,sod.price,sod.discountper,sod.discountamt,sod.taxable,sod.sgsttaxamt,sod.cgsttaxamt,sod.igsttaxamt,sod.finalprice,sod.iscancel
				...
				$htmldata.='<div class="col-lg-12 col-md-12 col-12 layout-spacing">';
					$htmldata.='<div class="widget mt-10">';
						$htmldata.='<div class="widget-content row">';
							...
							$htmldata.='<th style="text-align: center;">Total Qty</th>';
							$htmldata.='<th style="text-align: center;">Issued Qty</th>';
							$htmldata.='<th style="text-align: center;">Remain Qty</th>';
							...
							$htmldata.='<th style="text-align: right;">Item Status</th>';
							$htmldata.='</tr>';
							$htmldata.='</thead>';
							...
							{	
								...
								$htmldata.='<tr>';
								$htmldata.='<td align="left">'.$IISMethods->sanitize($subrow['itemname']).'<br><small>('.$IISMethods->sanitize($subrow['category']).')</td>';
								...
								$htmldata.='<td align="center">'.$IISMethods->sanitize($subrow['issuedqty']).'</td>';
								$htmldata.='<td align="center">'.$IISMethods->sanitize($subrow['remainqty']).'</td>';
								...
								$totalqty+=$subrow['qty'];
								$totalissuedqty+=$subrow['issuedqty'];
								$totalremainqty+=$subrow['remainqty'];
							...
							$htmldata.='<td align="center"><b>'.$totalissuedqty.'</b></td>';
							$htmldata.='<td align="center"><b>'.$totalremainqty.'</b></td>';
							$htmldata.='<td align="right"><b>'.$IISMethods->ind_format($totalprice).'</b></td>';
							$htmldata.='<td align="right"><b>'.$IISMethods->ind_format($totaldiscountamt).'</b></td>';
							$htmldata.='<td align="right"><b>'.$IISMethods->ind_format($totaltaxable).'</b></td>';
							$htmldata.='<td align="right"><b>'.$IISMethods->ind_format($totaltaxamt).'</b></td>';
							$htmldata.='<td align="right"><b>'.$IISMethods->ind_format($totalfinalprice).'</b></td>';
							$htmldata.='</tr>';
