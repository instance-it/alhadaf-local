...
require_once dirname(__DIR__, 2).'\model\state.php';
...
if($isvalidUser['status'] == 1) 
{
	...
	if($action=='insertstate')   
	{
		$formevent=$IISMethods->sanitize($_POST['formevent']);
		$state=$IISMethods->sanitize($_POST['state']);
		...
		{			
			...
			{
				$qrychk="SELECT state from tblstatemaster where state=:state";
				$parms = array(
					':state'=>$state,
				);
				...
				else
				{
					try 
					{
						...
						$DB->executedata('i','tblstatemaster',$insqry,'');
			...
			{
				$qrychk="SELECT state from tblstatemaster where state=:state AND id<>:id";
				$parms = array(
					':state'=>$state,
					':id'=>$id,
				);
				...
				else
				{
					try 
					{
						...
						$DB->executedata('u','tblstatemaster',$insqry,$extraparams);
	...
	else if($action=='deletestatemaster')   
	{
		$id=$IISMethods->sanitize($_POST['id']);
		$bulkid=$_POST['bulkid'];
		$bulk=explode (",", $bulkid); 
		if($id)
		{
			...
			else
			{
				try 
				{
					...
					$DB->executedata('d','tblstatemaster','',$extraparams);
					$status=1;
	...
	else if($action=='editstatemaster')   
	{
		$id=$IISMethods->sanitize($_POST['id']);
		if($id)
		{
			$qrychk="select id,state from tblstatemaster where id=:id";
			$parms = array(
				':id'=>$id,
			);
			$result_ary=$DB->getmenual($qrychk,$parms);
			...
			if(sizeof($result_ary)>0)
			{
				...
				$response['state']=$IISMethods->sanitize($row['state']);
	...
	else if($action=='liststatemaster')   
	{
		$states=new state();
		$qry="select timestamp as primary_date,id,state from tblstatemaster where state like :filter order by $ordbycolumnname $ordby offset $start rows fetch next $per_page rows only";
		$parms = array(
			':filter'=>$filter,
		);
		$states=$DB->getmenual($qry,$parms,'state');
		...
		if($responsetype=='HTML')
		{
			if($states)
			{
				...
				foreach($states as $state)
				{
					...
					$htmldata.='<input type="checkbox" class="custom-control-input bulkdelete" id="bulkdelete'.$i.'" value="'.$IISMethods->sanitize($state->id).'" name="bulkdelete[]">';
					$htmldata.='<label class="custom-control-label mb-0" for="bulkdelete'.$i.'"></label>';
					$htmldata.='</div>';
					$htmldata.='</div>';
					$htmldata.='</td>';
					$htmldata.='<td class="tbl-name">'.$IISMethods->sanitize($state->state,'OUT').'</td>';
					$htmldata.='</tr>';
					$i++;
				}
				$status=1;
		...
		{
			$response['data']= json_encode($states);
		}


		$common_listdata=$states;
	}    
}
