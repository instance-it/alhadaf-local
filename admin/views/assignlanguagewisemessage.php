        ...
        <div class="col-12 layout-spacing" id="layoutContent">
            ...
            <div class="widget widget-page">
                ...
                <div class="widget-content">
                    <form class="text-left validate-form" id="assignlanguagewisemessageForm" method="post" action="assignlanguagewisemessage" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-12 min-height-100">                                                
                                <div class="table-responsive main-grid">
                                    <table id="tableDataList" class="table table-bordered table-hover table-striped datalisttable table-search">
                                        <thead>
                                            <tr>
                                                <th class="tbl-name" width="15%">Message Name ID</th>
                                                <th class="tbl-name" width="35%">Message English Name</th>
                                                <th class="tbl-name" width="10%">Variables</th>
                                                <th class="tbl-name" width="40%">Message Name In <span class="textlanguage"></span></th>
                                            </tr>
                                        </thead>
                                        <tbody id='tabledata'>
...
<!-- CONTENT AREA -->
    ...
    <script>
        ...
        function resetdata()
        {
            $("#assignlanguagewisemessageForm").validate().resetForm();
            $('#assignlanguagewisemessageForm').trigger("reset");
        ...
        function filllangwisemsgdata()
        {
            ...
            formdata = new FormData();
            formdata.append("action", "filllangwisemsgdata");
			formdata.append("languageid", languageid);
            formdata.append("ordby", ordby);
            formdata.append("ordbycolumnname", ordbycolumnname);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessfilllangwisemsgdata,OnErrorfilllangwisemsgdata);
        }

        function Onsuccessfilllangwisemsgdata(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);
        ...
        function OnErrorfilllangwisemsgdata(content)
		{
        ...
        if($('#assignlanguagewisemessageForm').length){		
            $('#assignlanguagewisemessageForm').validate({
                rules:{
                    languageid:{
                        required: true,			
                    },
