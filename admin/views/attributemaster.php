...
?>
										...
										<table id="tableDataList" data-show="1" data-nextpage='0' class="table table-bordered table-hover table-striped datalisttable">
											<thead>
												<tr>
													...
													<th class="tbl-name sorting sorting_asc" data-th="attributename">Attribute Name</th>
					...
					<div class="right-sidebar-content">
						<div class="row">   
							...
							<div class="col-lg-12 col-md-12 col-12">
								<div class="widget">
									<div class="widget-content">
										<form class="row validate-form" id="attributemasterForm" method="post" action="attributemaster" enctype="multipart/form-data">
											<input type="hidden" id="formevent" name="formevent" value='addright'>
											<input type="hidden" id="id" name="id" value=''>	
											<div class="col-12">
												<div class="input-group">
													<label class="mb-1">Attribute Name <span class="text-danger">*</span></label>
													<label class="ml-auto "></label>
												</div>
												<div class="input-group mb-3">
													<input type="text" class="form-control" placeholder="Attribute Name" id="attributename" name="attributename" require>
												</div>
											</div>
											<div class="col-12">
												<div class="input-group">
	...
	<script>
		...
		function resetdata()
        {
            $("#attributemasterForm").validate().resetForm();
            $('#attributemasterForm').trigger("reset");
        ...
        function editdata(id)
        {
            ...
            formdata.append("action", "editattributemaster");
            formdata.append("id", id);

            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessattribute,OnErrorattribute); 
        }

        function Onsuccessattribute(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);

            $('#id').val(resultdata.id);
            $('#attributename').val(resultdata.attributename);  
        ...
        function OnErrorattribute(content)
        { 
		...
		if($('#attributemasterForm').length){		
            $('#attributemasterForm').validate({
                rules:{
                    attributename:{
                        required: true,			
                    },
                ...
                submitHandler: function(form){
                    ...
                    formdata.append("action", "insertattribute");
		...
		function changeattributestatus(id)
        {
            ...
            var headersdata= {Accept: 'application/json',platform: 1,userpagename:pagename,useraction:'editright',masterlisting:false};
            formdata = new FormData();
            formdata.append("action", "changeattributestatus");
            formdata.append("id", id);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccesschangeattributestatus,OnErrorchangeattributestatus); 
        }

        function Onsuccesschangeattributestatus(data)
        {
        ...
        function OnErrorchangeattributestatus(content)
        { 
            ajaxrequest("POST","'<?php echo $config->getPage404url(); ?>'",'','',OnsuccessRender,OnErrorRender);
        }
