...
?>
	<div class="layout-px-spacing">
			...
			<div class="col-12 layout-spacing d-none">
				<div class="widget">
					<div class="widget-heading m-0">
						<div class="row">
							<div class="col">
								<h5 class="my-2">Gallery Master</h5>
							</div>
										...
										<table id="tableDataList" data-show="1" data-nextpage='0' class="table table-bordered table-hover table-striped datalisttable">
											<thead>
												<tr>
													...
													<th class="tbl-name sorting sorting_asc" data-th="gcm.category">Category Name</th>
													<th class="tbl-name sorting " data-th="gm.title">Title</th>
					...
					<div class="right-sidebar-content">
						<div class="row">   
							...
							<div class="col-lg-12 col-md-12 col-12">
								<div class="widget">
									<div class="widget-content">
										<form class="row validate-form" id="gallerymasterForm" method="post" action="gallerymaster" enctype="multipart/form-data">
											...
											<div class="col-12">
												...
												<div class="input-group mb-3">
													<input type="text" class="form-control" placeholder="Title" id="title" name="title" require>
												</div>
											</div>
		...
		<!-- Images View Modal -->
		<div class="modal animated fade" id="GalleryImgModal" tabindex="-1" role="dialog" aria-labelledby="modalImgViewLabel" aria-hidden="true">
					...
					<div class="modal-body">
						<div class="row mb-4">
							<div class="col-12">
								...
								<h5 class="add-event-title modal-title">Gallery Image</h5>
							</div>
						</div>
						<div class="row">
	...
	<script>
		...
		function resetdata()
        {
            $("#gallerymasterForm").validate().resetForm();
            $('#gallerymasterForm').trigger("reset");
        ...
        function editdata(id)
        {
            ...
            formdata = new FormData();
            formdata.append("action", "editgallerymaster");
        ...
        function OnsuccessSubbrandedit(content)
        {
            ...
            formdata.append("action", "fillcategory");
			formdata.append("selectoption", 1);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessedtfillcategory,OnErrorSelectpicker);
		...
		function viewgalleryimage(imgfullurl)
        {
			$('#GalleryImgModal .clscatimg').attr('src',imgfullurl);
			$('#GalleryImgModal .clscatimglink').attr('href',imgfullurl);
		...
		function changegallerystatus(id)
        {
            ...
            var headersdata= {Accept: 'application/json',platform: 1,userpagename:pagename,useraction:'editright',masterlisting:false};
            formdata = new FormData();
            formdata.append("action", "changegallerystatus");
            formdata.append("id", id);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccesschangegallerystatus,OnErrorchangegallerystatus); 
        }

        function Onsuccesschangegallerystatus(data)
        {
        ...
        function OnErrorchangegallerystatus(content)
        { 
            ajaxrequest("POST","'<?php echo $config->getPage404url(); ?>'",'','',OnsuccessRender,OnErrorRender);
        }

		function changegalleryhomestatus(id)
		{
            var pagename=getpagename();
            var headersdata= {Accept: 'application/json',platform: 1,userpagename:pagename,useraction:'editright',masterlisting:false};
            formdata = new FormData();
            formdata.append("action", "changegalleryhomestatus");
            formdata.append("id", id);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccesschangegalleryhomestatus,OnErrorchangegalleryhomestatus); 
        }

        function Onsuccesschangegalleryhomestatus(data)
        {
            var JsonData = JSON.stringify(data);
            var resultdata = jQuery.parseJSON(JsonData);
            if(resultdata.status==0)
        ...
        function OnErrorchangegalleryhomestatus(content)
        { 
            ajaxrequest("POST","'<?php echo $config->getPage404url(); ?>'",'','',OnsuccessRender,OnErrorRender);
        }

		if($('#gallerymasterForm').length){		
            $('#gallerymasterForm').validate({
                rules:{
					categoryid:{
						required: true,			
					},
                ...
                submitHandler: function(form){
                    ...
                    formdata.append("action", "insertgallerymaster");
