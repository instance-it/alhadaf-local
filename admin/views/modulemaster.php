...
?>
                                    ...
                                    <table id="tableDataList" data-show="1" data-nextpage='0' class="table table-bordered table-hover table-striped datalisttable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="tbl-name sorting" data-th="moduletypetxt">Module Type</th>
                                                <th class="tbl-name sorting" data-th="modulename">Module Name</th>
                                            </tr>
                                        </thead>
                                        <tbody id="datalist">
                                        </tbody>
                ...
                <div class="right-sidebar-content">
                    <div class="row">   
                        ...
                        <div class="col-lg-12 col-md-12 col-12">
                            <div class="widget">
                                <div class="widget-content">
                                    <form class="row validate-form" id="modulemasterForm" method="post" action="modulemaster.php" enctype="multipart/form-data">
                                        ...
                                        <div class="col-12 ">
                                            ...
                                            <div class="input-group mb-3">
                                                <select class="form-control selectpicker" data-live-search="true" id="moduletypeid" name="moduletypeid" data-size="10" data-dropup-auto="false">
                                        ...
                                        <div class="col-12">
                                            <div class="input-group mb-0">
                                                <label class="mb-1">Module Name <span class="text-danger">*</span></label>
                                            </div>
                                            <div class="input-group mb-3">
                                                <input type="text" class="form-control" placeholder="Module Name" id="modulename" name="modulename">
                                            </div>
                                        </div>
                                        <div class="col-auto ml-auto">
                                            <div class="input-group mb-0">
                                                <button type="submit" class="btn btn-primary m-0" id="btnsubmit"><?php echo $config->getSaveSidebar(); ?></button>
                                                <button class="btn btn-secondary m-0 ml-2" id="btnReset" onclick="resetdata()"><?php echo $config->getResetSidebar(); ?></button>
                                            </div>
                                        </div>
                                    </from>
                                </div>
    ...
    <script>
        ...
        function resetdata()
        {
            $("#modulemasterForm").validate().resetForm();
            $('#modulemasterForm').trigger("reset");
        ...
        function editdata(id)
        {
            ...
            formdata.append("action", "editmodulemaster");
            formdata.append("id", id);

            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,OnsuccessModuleedit,OnErrorModuleedit); 
        }

        function OnsuccessModuleedit(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);

            $('#id').val(resultdata.id);
            $('#moduletypeid').val(resultdata.moduletypeid);
            $('#moduletypeid').selectpicker('refresh');
            $('#modulename').val(resultdata.modulename);  
        ...
        function OnErrorModuleedit(content)
        { 
            ajaxrequest("POST","'<?php echo $config->getPage404url(); ?>'",'','',OnsuccessRender,OnErrorRender);
        }
        //Edit End

        if($('#modulemasterForm').length){		
            $('#modulemasterForm').validate({
                rules:{
                    moduletypeid:{
                        required: true,			
                    },
                ...
                submitHandler: function(form){
                    ...
                    formdata.append("action", "insertmodulemaster");
