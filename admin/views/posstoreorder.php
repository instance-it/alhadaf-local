...
?>
				...
				<div class="right-sidebar-content">   
					<div class="row">   
						...
						<div class="col-lg-12 col-md-12 col-12">
							<form class="form" action="" name="fltposstoreorderFrm" id="fltposstoreorderFrm">
	...
	<script>
		...
		//View Store Order Data Start
        function viewstoreorderdata(id)
        {
            ...
            formdata.append("action", "viewstoreorderdata");
            formdata.append("id", id);

            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessviewstoreorderdata,OnErrorviewstoreorderdata); 
        }

		function Onsuccessviewstoreorderdata(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);
            $('#GeneralModal').modal('show');
			$('#GeneralModal .modal-dialog').addClass('modal-exl');
			$('#GeneralModal #GeneralModalLabel').html('Store Order Details');
			$('#GeneralModal #Generaldata').html(resultdata.data);
        }
        
        function OnErrorviewstoreorderdata(content)
        { 
		...
		{
			$("#fltposstoreorderFrm").validate().resetForm();
            $('#fltposstoreorderFrm').trigger("reset");
			fillfltstore();
		}

		$('body').on('click','#btnfiltersave',function(){
			$('#fltposstoreorderFrm #issidebarflt').val(1);
