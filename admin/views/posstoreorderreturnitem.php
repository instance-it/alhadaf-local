...
?>
										...
										<table id="tableDataList" data-show="1" data-nextpage='0' class="table table-bordered table-hover table-striped datalisttable">
											<thead>
												...
												<th class="tbl-name sorting sorting_asc" data-th="s.storename">Store</th>
													<th class="tbl-name sorting sorting_asc" data-th="pm.personname">Member</th>
													<th class="tbl-name sorting" data-th="sro.timestamp">Return Date</th>
													<th class="tbl-name sorting" data-th="pm1.personname">Return By</th>
				...
				<div class="right-sidebar-content">   
					<div class="row">   
						...
						<div class="col-lg-12 col-md-12 col-12">
							<form class="form" action="" name="fltposstoreorderreturnitemFrm" id="fltposstoreorderreturnitemFrm">
	...
	<script>
		...
		//View Store Order Return Data Start
        function viewreturnorderdata(id)
        {
            ...
            formdata.append("action", "viewreturnorderdata");
            formdata.append("id", id);

            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessviewreturnorderdata,OnErrorviewreturnorderdata); 
        }

		function Onsuccessviewreturnorderdata(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);
            $('#GeneralModal').modal('show');
			$('#GeneralModal .modal-dialog').addClass('modal-exl');
			$('#GeneralModal #GeneralModalLabel').html('Store Return Item Details');
			$('#GeneralModal #Generaldata').html(resultdata.data);
        }
        
        function OnErrorviewreturnorderdata(content)
        { 
		...
		{
			$("#fltposstoreorderreturnitemFrm").validate().resetForm();
            $('#fltposstoreorderreturnitemFrm').trigger("reset");
		}

		$('body').on('click','#btnfiltersave',function(){
			$('#fltposstoreorderreturnitemFrm #issidebarflt').val(1);
