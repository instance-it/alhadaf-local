...
?>
					...
					<div class="right-sidebar-content">
						<div class="row">   
							...
							<div class="col-lg-12 col-md-12 col-12">
								<div class="widget">
									<div class="widget-content">
										<form class="row validate-form" id="rangemasterForm" method="post" action="instructionmaster" enctype="multipart/form-data">
											...
											<div class="col-12">
												...
												<div class="input-group mb-3">
													<input type="text" class="form-control" placeholder="Range Name" id="rangename" name="rangename" require />
												</div>
											</div>
											...
											<div class="col-12">
                                                ...
                                                <div class="input-group mb-3 mb-lg-3">
                                                    <select class="form-control selectpicker" data-live-search="true" multiple="multiple" id="laneid" name="laneid[]" data-size="10" data-dropup-auto="false" title="Select Lane">
	...
	<script>
		...
		function filllane()
        {
            ...
            formdata.append("action", "filllane");
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+'master',formdata,headersdata,Onsuccessfilllane,OnErrorSelectpicker);
        }
        function Onsuccessfilllane(content)
        {
        ...
        function editdata(id)
        {
            ...
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessrangemaster,OnErrorrangemaster); 
        }

        function Onsuccessrangemaster(content)
        {
            var JsonData = JSON.stringify(content);
            ...
            formdata = new FormData();
            formdata.append("action", "filllane");
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+'master',formdata,headersdata,Onsuccessedtfilllane,OnErrorSelectpicker);

			function Onsuccessedtfilllane(content)
			{
				var JsonData = JSON.stringify(content);
				var resultdata = jQuery.parseJSON(JsonData);
				$('#laneid').html(resultdata.data);
                ...
                {
                    var bt=resulteditdata.laneid.split(",");
                    $('#laneid').selectpicker('val',bt);
                }
				$('#laneid').selectpicker('refresh');	
			}
        ...
        function OnErrorrangemaster(content)
        { 
		...
		//View Details Start
        function viewrangedetaildata(id)
        {
            var pagename=getpagename();
            ...
            formdata = new FormData();
            formdata.append("action", "viewrangedetaildata");
            formdata.append("id", id);

            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+pagename,formdata,headersdata,Onsuccessviewrangedetaildata,OnErrorviewrangedetaildata); 
        }

		function Onsuccessviewrangedetaildata(content)
        {
            var JsonData = JSON.stringify(content);
            var resultdata = jQuery.parseJSON(JsonData);
            $('#GeneralModal').modal('show');
			$('#GeneralModal .modal-dialog').addClass('modal-xl');
			$('#GeneralModal #GeneralModalLabel').html('Lane');
			$('#GeneralModal #Generaldata').html(resultdata.data);
        }
        
        function OnErrorviewrangedetaildata(content)
        { 
            ajaxrequest("POST",'<?php echo $config->getPage404url(); ?>','','',OnsuccessRender,OnErrorRender);
        }
		...
		if($('#rangemasterForm').length){		
            $('#rangemasterForm').validate({
                ...
                submitHandler: function(form){
                    ...
                    formdata.append("action", "insertrange");
