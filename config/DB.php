...
function SAPLoginAPIData($SubDB)    
{
	...
	curl_setopt_array($curl, array(
	  ...
	  CURLOPT_HEADERFUNCTION =>     function ($curl, $string) use (&$routeId) {
		$len = strlen($string);
	...
	curl_close($curl);
	self::InsertCurlApiLogData('Login',$CURLOPT_URL,$CURLOPT_POSTFIELDS,$response,$httpcode,$total_time);
	
	
	
	$response1=json_decode($response,true);
	
	$responsedata['SessionId']=$response1['SessionId'];              
	$responsedata['RouteId']=$routeId; 
		...
		if(sizeof($resord) > 0)
		{
			...
			if(sizeof($resod) > 0)
			{
				...
				for($i=0;$i<sizeof($resod);$i++)
				{
					...
					if($rowod['sapitemid'] != '')
					{
						$SAPData['DocumentLines'][$j]['ItemCode']=$rowod['sapitemid'];              
						$SAPData['DocumentLines'][$j]['Quantity']="1";              
						$SAPData['DocumentLines'][$j]['UoMEntry']="1";              
						$SAPData['DocumentLines'][$j]['VatGroup']=$rowod['saptaxid'];              
						$SAPData['DocumentLines'][$j]['UnitPrice']=$rowod['price']; 
			...
			if($resInvoiceData['DocEntry'] != '')
			{
				...
				$updqry=array(					
					...
					'sapdocnum'=>$resInvoiceData['DocNum'],					
				);
				$extraparams=array(
					'id'=>$orderid
				);
...
function SAPInsertARInvoiceDeliveryData($SubDB,$orderid)    
{
	...
	if($resLoginData['SessionId'] != '')	
	{
		...
		$qryord="SELECT distinct so.*,pm.sappersonid,pm.personname,isnull(pm.address,'') as pm_address,isnull(pm.email,'') as pm_email,convert(date,so.timestamp,103) as ord_date 
		...
		if(sizeof($resord) > 0)
		{
			...
			if(sizeof($resod) > 0)
			{
				...
				for($i=0;$i<sizeof($resod);$i++)
				{
					...
					{
						$SAPData['DocumentLines'][$j]['ItemCode']=$rowod['sapitemid'];              
						$SAPData['DocumentLines'][$j]['Quantity']=$rowod['qty'];              
						$SAPData['DocumentLines'][$j]['PackageQuantity']=$rowod['qty'];              
						$SAPData['DocumentLines'][$j]['UoMEntry']="1";              
						$SAPData['DocumentLines'][$j]['VatGroup']=$rowod['saptaxid'];              
						$SAPData['DocumentLines'][$j]['Price']=$rowod['price'];   
			...
			$CURLOPT_URL=$config->getCurlSAPapiurl().'DeliveryNotes';
			$CURLOPT_CUSTOMREQUEST='POST';
			$CURLOPT_POSTFIELDS=$SAPDataJosn;
			...
			$resDeliveryData=$IISMethods->FnCURLResponse($CURLOPT_URL,$CURLOPT_CUSTOMREQUEST,$CURLOPT_POSTFIELDS,$CURLOPT_HTTPHEADER,'Delivery (Item Issued)');


			if($resDeliveryData['DocEntry'] != '')
			{
				//Update Order Data
				$updqry=array(					
					'saporderid'=>$resDeliveryData['DocEntry'],					
				);
				$extraparams=array(
					'id'=>$orderid
				);
...
function SAPInsertARInvoiceReturnData($SubDB,$orderid)    
{
	...
	if($resLoginData['SessionId'] != '')	
	{
		...
		if(sizeof($resord) > 0)
		{
			...
			if(sizeof($resod) > 0)
			{
				...
				for($i=0;$i<sizeof($resod);$i++)
				{
					...
					if($rowod['sapitemid'] != '')
					{
						...
						$SAPData['DocumentLines'][$j]['UoMEntry']="1";              
						$SAPData['DocumentLines'][$j]['VatGroup']=$rowod['saptaxid'];              
						$SAPData['DocumentLines'][$j]['UnitPrice']='0';   
			...
			$CURLOPT_URL=$config->getCurlSAPapiurl().'Returns';
			$CURLOPT_CUSTOMREQUEST='POST';
			$CURLOPT_POSTFIELDS=$SAPDataJosn;
			...
			$resReturnsData=$IISMethods->FnCURLResponse($CURLOPT_URL,$CURLOPT_CUSTOMREQUEST,$CURLOPT_POSTFIELDS,$CURLOPT_HTTPHEADER,'Returns (Item Return)');


			if($resReturnsData['DocEntry'] != '')
			{
				//Update Order Data
				$updqry=array(					
					'saporderid'=>$resReturnsData['DocEntry'],					
				);
				$extraparams=array(
					'id'=>$orderid
				);
