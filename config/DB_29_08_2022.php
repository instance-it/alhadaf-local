...
function SAPInsertMemberData($SubDB,$memberid)    
{
	...
	if(sizeof($resmem) > 0)
	{
		...
		$unqCode = self::SAPRandomMemberCode($SubDB,8);

		/*
		$qrysap = "SELECT \"CardCode\" from ".$saptablename." where \"Cellular\"='$rowmem[contact]' or \"E_Mail\"='$rowmem[email]' ";
		$ressap=$SubDB->getmenual($qrysap);
		...
		if(sizeof($ressap) > 0)
		{
			...
			$unqCode = $rowsap['CardCode'];
		}
		else
		{
			$unqCode = self::SAPRandomMemberCode($SubDB,8);
			$DocEntryNum = self::SAPGetMaxDocEntryNumber($SubDB);


			$qry="INSERT INTO ".$saptablename." (\"CardCode\",\"CardName\",\"CardType\",\"GroupCode\",\"CmpPrivate\",\"Address\",\"MailAddres\",\"Phone1\",\"Currency\",\"Cellular\",\"E_Mail\",\"CardFName\",\"DocEntry\",\"DebPayAcct\",\"validFor\",\"CreateDate\",\"UpdateDate\",\"UseShpdGd\",\"ListNum\") 
				values ('$unqCode','$rowmem[personname]','C','100','C','$rowmem[address]','$rowmem[email]','$rowmem[contact]','QAR','$rowmem[contact]','$rowmem[email]','$rowmem[personname]','$DocEntryNum','140000','Y','$sapentrydate','$sapentrydate','N','1')";
...
function SAPInsertARInvoiceData($SubDB,$orderid)    
{
	...
	if(sizeof($resord) > 0)
	{
		...
		ifnull((select top 1 \"Ref1\" from ".$SubDBName.".OINV where \"Handwrtten\"='N' order by \"DocEntry\" desc),0) as \"Ref1\",
		ifnull((select \"Series\" from ".$SubDBName.".NNM1 where \"ObjectCode\"='13' and \"Locked\"='N'),0) as \"Series\"  
		from ".$SubDBName.".OINV where \"Handwrtten\"='N'";
		$ressap=$SubDB->getmenual($qrysap);
		$rowsap=$ressap[0];
