...
<!------------- Start For Website Label Array  -------------------->
<script type="text/javascript">
var webenglishlabelarr = {
    "lan-home-menu": "Home",
    "lan-course-menu": "Courses",
    "lan-premembership-menu": "Premium Membership",
    "lan-specialpackage-menu": "Special Packages",
    "lan-rangebook-menu": "Range Booking",
    "lan-aboutus-menu": "About Us",
    "lan-gallery-menu": "Gallery",
    "lan-contactus-menu": "Contact Us",

    "lan-howtofindus-menu": "How to Find Us",
    "lan-googlemap-menu": "GOOGLE MAPS",
    "lan-ourequipment-menu": "Our Equipment",

    "lan-footer-signup": "Sign Up",
    "lan-footer-signupreceivefrom": "Sign Up to Receive Specials from",
    "lan-footer-readagree": "I have read and agree to the",
    "lan-footer-tandc": "terms & conditions",
    "lan-footer-rangehour": "Range Hours",

    "lan-footer-customerservice": "Customer Service",
    "lan-footer-simulator": "Simulator",
    "lan-footer-rifle": "Rifle",
    "lan-footer-airgun": "Airgun",
    "lan-footer-skeet": "Skeet",
    "lan-footer-personaltraining": "Personal Training / Courses",
    "lan-footer-foodbeverage": "Food and Beverage",
    "lan-footer-faq": "FAQ",
    "lan-footer-careeropportunity": "Career Opportunities",
    "lan-footer-champion": "Al Hadaf Champion & Tournament",
};


var webarabiclabelarr = {
    "lan-home-menu": "الصفحة الرئيسية",
    "lan-course-menu": "الدورات",
    "lan-premembership-menu": "عضوية مميزة",
    "lan-specialpackage-menu": "باقات خاصة",
    "lan-rangebook-menu": "الحجز مضمار",
    "lan-aboutus-menu": "معلومات عنا",
    "lan-gallery-menu": "معرض",
    "lan-contactus-menu": "اتصل بنا",

    "lan-howtofindus-menu": "كيف تجدنا",
    "lan-googlemap-menu": "خرائط غوغل",
    "lan-ourequipment-menu": "معداتنا",

    "lan-footer-signup": "اشتراك",
    "lan-footer-signupreceivefrom": "قم بالتسجيل لتلقي العروض الخاصة من",
    "lan-footer-readagree": "لقد قرأت ووافقت على ال",
    "lan-footer-tandc": "الشروط وا",
    "lan-footer-rangehour": "ساعات الحارات",

    "lan-footer-customerservice": "خدمة الزبائن",
    "lan-footer-simulator": "محاكي",
    "lan-footer-rifle": "بندقية",
    "lan-footer-airgun": "بندقية الهواء",
    "lan-footer-skeet": "سكيت",
    "lan-footer-personaltraining": "التدريب الشخصي / الدورات",
    "lan-footer-foodbeverage": "الأغذية والمشروبات",
    "lan-footer-faq": "التعليمات",
    "lan-footer-careeropportunity": "فرص عمل",
    "lan-footer-champion": "بطولة الهدف",
};

</script>
<!------------- End For Website Label Array  -------------------->
...
<script type="text/javascript"> 
    function googleTranslateElementInit() { 
        ...
        setTimeout(function(){ 
            ...
            var langcode = $("#google_translate_element select").val();
            //alert(langcode);
            if(langcode == '')
            {
                langcode='en';
            }
            $('.navbar-country .dropdown-item-'+langcode).click();
    ...
    //Set Website Language Wise Label
    function setwebsitelanguagelabel() {
        var language = $('.asrlanguage').attr('data-language');

        //alert(language);
        
        if(language == 'ar')  //For arabic
        {
            $.each( webarabiclabelarr, function( key, value ) {
                $("."+key).text(value);
            });
        }
        else  //For English
        {
            $.each( webenglishlabelarr, function( key, value ) {
                $("."+key).text(value);
            });
        }  
    }
...
new When({
    input: document.getElementById('r_passportidexpiry'),
    singleDate: true,
    outputFormat: "DD/MM/YYYY",
    showHeader: false,
});
...
function Onsuccessmemberfpass(content)
{
    ...
    else if(resultdata.status==1)
    {
        ...
        resetpassdiv+='<div class="col-12">';
        resetpassdiv+='<div class="form-group validate-input">';
        resetpassdiv+='<input class="form-control fr_newpass" id="fr_newpass" name="fr_newpass" type="password" required="required">';
        resetpassdiv+='<span class="focus-form-control"></span>';
...
var googleUser = {};
  var startApp = function() 
  {
    gapi.load('auth2', function()
    {
      ...
      attachSignin(document.getElementById('customGoogleBtn1'));
      attachSignin(document.getElementById('customGoogleBtn2'));
    });
  };
startApp();
  function attachSignin(element) {
    ...
    auth2.attachClickHandler(element, {},
        function(googleUser) 
        {
            var logtype = element.type;    //1-Register, 2-Login
            //alert(logtype);
            ...
            formdata.append("webid", googleUser.getBasicProfile().getId());
            formdata.append("logtype", logtype);
            formdata.append("targeturl", targeturl);
            ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+'login',formdata,headersdata,OnsuccessgSocialLogin,OnErrorgSocialLogin);


            function OnsuccessgSocialLogin(content)
            {
                var JsonData = JSON.stringify(content);
                var resultdata = jQuery.parseJSON(JsonData);
            ...
            function OnErrorgSocialLogin(content)
            {
                var JsonData = JSON.stringify(content);
                var resultdata = jQuery.parseJSON(JsonData);
...
function Onsuccessaddtocartitem(content)
{
    ...
    else if(resultdata.status==1)
    {
        if(resultdata.isbuynow == 1)
        {
            ...
            $('#fullspecificationModal').modal('hide');
...
/************************* Start For Website Logout ******************************/
function logoutwebsitepage()
{
    $("#preloader").fadeIn();
    currentXhr.abort();
    ...
    ajaxrequest("POST",'<?php echo $config->getEndpointurl(); ?>'+'logout',formdata,headersdata,OnsuccesswebsiteLogout,OnErrorwebsitelogout);
}

function OnsuccesswebsiteLogout(content)
{
    $("#preloader").fadeOut();
    var JsonData = JSON.stringify(content);
    var resultdata = jQuery.parseJSON(JsonData);
...
function OnErrorwebsitelogout(content) {
    $("#preloader").fadeOut();
}
/************************* End For Website Logout ******************************/
