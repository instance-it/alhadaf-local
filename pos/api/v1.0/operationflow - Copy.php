...
if($isvalidUser['status'] == 1) 
{
	...
	{	
		...
		if($sopid && $fromtime && $totime)
		{
			...
			rl.id not in (
				...
				where isnull(ar.isreleased,0)=0
				and (
					(TRY_PARSE(concat(:fromdate,' ',:fromtime) AS DATETIME USING 'en-gb') between TRY_PARSE(concat(ar.date,' ',ar.fromtime) AS DATETIME USING 'en-gb') and TRY_PARSE(concat(ar.date,' ',ar.totime) AS DATETIME USING 'en-gb'))
					or
					(TRY_PARSE(concat(ar.date,' ',ar.fromtime) AS DATETIME USING 'en-gb') between TRY_PARSE(concat(:fromdate1,' ',:fromtime1) AS DATETIME USING 'en-gb') and TRY_PARSE(concat(:todate1,' ',:totime1) AS DATETIME USING 'en-gb'))
				)
			) 
	...
	else if($action == 'insertrangeassigndata')
	{
		...
		if($storeid && $memberid && $rangeid && $laneid && $fromtime && $totime)
		{
			//Check Store Operation Flow
			$qryso="select so.id,so.iscompulsory 
				from tblstoreoperation1 so 
			...
			if(sizeof($resso) > 0)
			{
				...
				{ 
					...
					from tblassignrangelane ar 
					where ar.rangeid=:rangeid and ar.laneid=:laneid and isnull(ar.isreleased,0)=0
					and (
						(TRY_PARSE(concat(:fromdate,' ',:fromtime) AS DATETIME USING 'en-gb') between TRY_PARSE(concat(ar.date,' ',ar.fromtime) AS DATETIME USING 'en-gb') and TRY_PARSE(concat(ar.date,' ',ar.totime) AS DATETIME USING 'en-gb'))
						or
						(TRY_PARSE(concat(ar.date,' ',ar.fromtime) AS DATETIME USING 'en-gb') between TRY_PARSE(concat(:fromdate1,' ',:fromtime1) AS DATETIME USING 'en-gb') and TRY_PARSE(concat(:todate1,' ',:totime1) AS DATETIME USING 'en-gb'))
	...
	else if($action == 'listassignedrangemember')
	{
		...
		if($date)
		{
			...
			$qry="SELECT distinct pm.id,isnull(pm.personname,'') as personname,isnull(pm.firstname,'') as firstname,isnull(pm.middlename,'') as middlename,isnull(pm.lastname,'') as lastname,isnull(pm.contact,'') as contact,isnull(pm.email,'') as email,isnull(pm.address,'') as address,
				...
				ar.id as rangeassignid,ar.date,ar.fromtime,ar.totime,isnull(ar.isreleased,0) as isreleased,case when (isnull(ar.isreleased,0)=1) then 'Released' else 'Assigned' end as releasestatus,case when (isnull(ar.isreleased,0)=1) then '#17a34f' else '#e6a825' end as releasestatuscolor
				FROM tblassignrangelane ar
				INNER JOIN tblpersonmaster pm on pm.id=ar.memberid
				WHERE isnull(pm.isdelete,0)=0 AND isnull(pm.isverified,0)=1 and convert(date,ar.date,103)=convert(date,:date,103)";
			$parms = array(
				':date'=>$date,
			...
			if(sizeof($memberdata) > 0)
			{
				...
				for($i=0;$i<sizeof($memberdata);$i++)
				{
					...
					$qryitem="select sod.itemid as id,REPLACE(sod.itemname,'&amp;','&') as name,sum(sod.qty) as qty
							...
							where so.status=1 and so.uid=:uid and convert(date,so.orderdate,103)=convert(date,:date,103) and sod.catid=:returnablecatid
